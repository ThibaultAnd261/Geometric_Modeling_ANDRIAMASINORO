...
void display() 
{
	...
	glUniformMatrix4fv(glGetUniformLocation(shaderprogram, "myprojection_matrix"), 1, GL_FALSE, &projection_matrix[0][0]);
	...
	glUniformMatrix4fv(glGetUniformLocation(shaderprogram, "myview_matrix"), 1, GL_FALSE, &view_matrix[0][0]);
	...
	// Autres infos (texte, stats, etc.)
	color = { 0.0f, 0.0f, 0.0f };
	draw_text(0.0f, 1.0f, 0, "Vertices: " + to_string(m->vertices.size()), color);
	draw_text(0.0f, 22.0f, 0, "Halfedges: " + to_string(m->halfedges.size()), color);
	draw_text(0.0f, 41.0f, 0, "Faces: " + to_string(m->faces.size()), color);
	color[0] = 0.9f;
	draw_text(Glut_w - 150.0f, Glut_h - 20.0f, 0, "FPS: " + to_string((int)fps), color);
